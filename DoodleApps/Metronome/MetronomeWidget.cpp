...
Doodle::MetronomeWidget::MetronomeWidget() : Widget(0) {
	makeVisual();
}

Doodle::MetronomeWidget::MetronomeWidget(Widget* parent = 0) : Widget(parent) {
	makeVisual();
}

void Doodle::MetronomeWidget::makeVisual() {
	_sides = 32;
	int i;
	for(i = 0; i < _sides; i++) {
		QPointF q;
...
void Doodle::MetronomeWidget::render(QPainter* painter) {
	painter->save();
	...
	// TODO: Make a label with the bpm.
//	int i;
